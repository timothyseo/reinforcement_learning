...
class GraphicDisplay(tk.Tk):
    ...
    def load_images(self):
        up = ImageTk.PhotoImage(Image.open(os.path.join(self.img_path, "up.png")).resize((13, 13)))
        right = ImageTk.PhotoImage(Image.open(os.path.join(self.img_path, "right.png")).resize((13, 13)))
        left = ImageTk.PhotoImage(Image.open(os.path.join(self.img_path, "left.png")).resize((13, 13)))
        down = ImageTk.PhotoImage(Image.open(os.path.join(self.img_path, "down.png")).resize((13, 13)))
        rectangle = ImageTk.PhotoImage(Image.open(os.path.join(self.img_path, "rectangle.png")).resize((65, 65)))
        triangle = ImageTk.PhotoImage(Image.open(os.path.join(self.img_path, "triangle.png")).resize((65, 65)))
        circle = ImageTk.PhotoImage(Image.open(os.path.join(self.img_path, "circle.png")).resize((65, 65)))
...
# env 객체에 정의된 변수와 함수
# env.width, env.height -> 그리드월드의 너비와 높이, 그리드월드의 가로와 세로를 정수로 반환
# env.state_after_action(state, action) -> 특정 상태에서 특정 행동을 했을 때 에이전트가 가는 다음 상태, 행동 후의 상태를 좌표로 표현한 리스트 반환 ex) [1,2]
# env.get_all_state -> 존재하는 모든 상태, 모든 상태를 반환 ex) [0,0], [0,1], [0,2]...
# env.get_reward(state, action) -> 특정 상태의 보상, 점수 형태로 보상을 반환
# env.possible_actions -> 상, 하, 좌, 우 -> [0,1,2,3] 반환, 순서대로 상, 하, 좌, 우 의미